...
def prediction(args):
    ...
    for i, batch in tqdm(enumerate(data_loader)):
        ...
        bin_score = []
        bin_preds = []
        for k, input_patch_batch in enumerate(patches):
            input_patch_batch = input_patch_batch.to(cfg.MODEL.DEVICE)
            with torch.no_grad():
                dim_pred, bin_score, bin_pred = posenet(input_patch_batch)
                print(dim_pred, bin_pred, bin_score)
            # box2d = torch.tensor(boxes[k])
            # print(dim_pred.shape)
            # corners = dimensions_to_corners(dim_pred)
            # theta_l = 
